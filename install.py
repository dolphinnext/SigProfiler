		...
		if not rsync:
			try:
				if bash:
					...
					try:
						os.system("bash -c '" + 'wget -r -l1 -c -nc --no-parent -nd -P ' + chromosome_fasta_path + ' https://bioinfo.umassmed.edu/share/dnext_data/genome_data/mouse/'+ genome + '/SigProfiler/' + genome + '.tar.gz 2>> install.log' + "'")
					except:
						print("The Sanger ftp site is not responding. Please check your internet connection/try again later.")
				else:
					os.system('wget -r -l1 -c -nc --no-parent -nd -P ' + chromosome_fasta_path + ' https://bioinfo.umassmed.edu/share/dnext_data/genome_data/mouse/' + genome + '/SigProfiler/' + genome + '.tar.gz 2>> install.log')
